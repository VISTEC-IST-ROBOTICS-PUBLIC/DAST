function sysCall_init()
    ...
    dataBaseKeys[9] = 'body_z'     -- Add this
    dataBaseKeys[10] = 'IMU_roll'
    dataBaseKeys[11] = 'IMU_pitch'
    dataBaseKeys[12] = 'IMU_yaw'
    ...
    robotPos_pub = simROS.advertise('sensor/robot_pos','std_msgs/Float64MultiArray')
    ...
    for i = 1,table.getn(rosnode),1 do
    	...
    	result=sim.launchExecutable( sim.getStringParameter(sim.stringparam_scene_path) .. '/../../../devel/lib/medaextra_pkg/'..rosnode[i], '/cpg_topic', 0)
    end
...
function sysCall_sensing()
    ...
    if first == 0 then
        ...
        camTop_init_angle = sim.getObjectOrientation(camera_bodyTrack_top, -1)
    ...
    -- body pos
    simROS.publish(robotPos_pub,{data=pos})
    ...
    body_angle = sim.getObjectOrientation(bodypartHandles[2], -1)
    ...
    sim.setObjectPosition(camera_bodyTrack_top, -1, {body_pos[1], body_pos[2], camTop_init_pos[3] + body_pos[3]}) -- use this
    sim.setObjectOrientation(camera_bodyTrack_top, -1, {camTop_init_angle[1], camTop_init_angle[2], -imu_deg[3]-math.pi/2})
    ...
    if is_visualize == true then
        ...
        -- pos_foot2 = sim.getObjectPosition(rightFoot[3], -1)
    ...
    pos =  sim.getObjectPosition(bodypartHandles[1], -1)
    ...
    if(logging and t > 1.0 ) then
        ...
        table.insert(dict['body_z'], pos[3]) -- add this
...
-- ******************** Time set *********************************************************************************-- 5000
    -- print("yaw = " .. yaw)
    -- Time limit
    print("time " .. t .. " || yaw = " .. yaw*180/math.pi)
    if(t > 81) then 
        sim.stopSimulation()
    end
...
function sysCall_cleanup()
    ...
    if(logging) then
        ...
        writeDataBaseToCsvFile('/data/fwdmodel/steering/')
