...
void adaptiveSF::stepNoLearning(vector<double> SensorData, double *target_pos, double simtime, double gamma_, double *imu_, double *robot_pos_){
    ...
    for(int i = 0; i < nOfSegments; i++) // --> for stick insect : nOfSegments = 1
    {
        for(int j = 0; j < legsPerSegment; j++)  // --> for stick insect : legsPerSegment = 6
        {
            ...
            for(int i = 0; i < 3; i++){
                ...
                cpgs[cpg_index].robot_pos[i] = robot_pos_[i];
            }
