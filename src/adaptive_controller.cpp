...
AdaptiveController::AdaptiveController(ros::NodeHandle &nh): nh_ (nh)
{
    ...
    get_robot_pos = nh.subscribe("sim_ros_interface/sensor/robot_pos",1, &AdaptiveController::getRobotPos, this);
...
void AdaptiveController::getImuSenser(const std_msgs::Float64MultiArray::ConstPtr& array){
    ...
    if(size == 3){
        ...
        ROS_INFO("ERROR IN IMU SENSOR X DATA SIZE!");
    }
}

// =========================================================================================== <<<<<<<<
void AdaptiveController::getRobotPos(const std_msgs::Float64MultiArray::ConstPtr& array){
    int size =  array->data.end() - array->data.begin(); 

    if(size == 3){
        for (size_t i = 0; i < size; i++)
        {
            get_robot_pos_value[i] = array->data[i]; 
        }
    }else{
        ROS_INFO("ERROR IN Robot Pos X DATA SIZE!");
    }
}
...
void AdaptiveController::stepAdaptiveSF(){
    ...
    controller->stepNoLearning(force_vec, target_pos, global_time, get_gamma_value, get_imu_value, get_robot_pos_value);
