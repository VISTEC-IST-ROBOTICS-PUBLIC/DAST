...
DualLearner::DualLearner(std::vector<double> w_, std::vector<double> learnParams_, bool closeLoop, int index, int rand){
    ...
    rbfNetSemicircle = new rbfn_semicircle(index, 0.2, 41);  // 20 -->>  41 Nine
...
std::vector<double> DualLearner::control_step(double forceSensorData, double time, bool is_set_gamma, double gamma){
    ...
    filteredData = filteredData > 2 ? 2 : filteredData; 
    ...
    motorOutput[13] =  C1; // roll_degree              // data[10]
    motorOutput[14] =  C2; //pitch_degree;             // data[11]
    ...
    std::vector<double> cpgOutput = {C1, C2}; // <----------- Nine


    std::vector<double> output;
    if(use_semicircle_RBF){ // << use semi for nine weight
    ...
    // ================================================================================ //
    // ***************************   Direction control DAST  ************************** //
    // ================================================================================ //

    bool is_direction_control = true;

    double target_direction = 0; // walk forward
    double theta_sat = 10;
    
    if (is_direction_control){
        error = target_direction - yaw_degree;
        direction_factor = (max(0, 1-(max(0,(error*-g_s))/10)));

        // ============================================ //
        // Disturb TC-joint command
        motorOutput[0] = (output[0]*direction_factor) + (offset_TCjoint*M_PI/180);
    }
...
std::vector<double> DualLearner::LearningModel_step(double forceSensorData, bool delay){
    ...
    // 2 fwdModel = Expected Sensory Data [coeff = factor between 0-1 defining shape of model]
    // Act like --> low pass filter -- coeff = 0.7
    coeff = 1.0;
    fwdMod = G;
...
double DualLearner::relu(double x)
{
    if (x < 0){
        return 0;
    }
